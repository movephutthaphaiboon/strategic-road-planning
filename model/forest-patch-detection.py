"""
Forest patch detection for Cameroon.

Outputs
-------
<stem>.tif   — country-level patch-ID raster (uint32, 0 = background)
<stem>.gpkg  — admin boundary vectors with fragmentation stats

Usage
-----
python forest-patch-detection.py
python forest-patch-detection.py --threshold 20 --min-patch-size 5 --admin-level 1
python forest-patch-detection.py --burn-paved
python forest-patch-detection.py --burn-paved --burn-unpaved
"""
...
_DEFAULT_TREECOVER = _REPO_ROOT / "data/output/processed-hansen-treecover/cameroon_treecover2024_30m.tif"
...
def load_treecover_90m(treecover_path: Path):
    ...
    with rasterio.open(treecover_path) as src:
        ...
        crs = src.crs
        nodata = src.nodata
    return arr, out_transform, crs, nodata
...
def make_forest_binary(arr: np.ma.MaskedArray, threshold: int):
...
def burn_roads(forest: np.ndarray, valid: np.ndarray,
               transform: Affine, crs,
               road_path: Path, road_types: list) -> tuple:
    """
    Set road pixels to non-forest (and non-valid) in both arrays.
    road_types: list of liu_surface values to burn, e.g. ['paved'] or ['paved', 'unpaved'].
    Returns updated (forest, valid) arrays.
    """
    ...
    forest = forest & ~road_mask
    valid = valid & ~road_mask
    print(f"  Burned {n_burned:,} road pixels ({n_burned * compute_pixel_area_ha(transform):.0f} ha)")
    return forest, valid


def label_patches_country(forest: np.ndarray, min_patch_px: int):
    """
    Label connected forest patches (8-connectivity) across the full country.
    Patches smaller than min_patch_px are removed (set to 0).
    Returns a compact uint32 label array.
    """
    labeled, _ = ndimage_label(forest, structure=_STRUCT_8)
    sizes = np.bincount(labeled.ravel())
    sizes[0] = 0  # background is never a patch
    too_small = (sizes > 0) & (sizes < min_patch_px)
    if too_small.any():
        remove_mask = too_small[labeled]
        labeled[remove_mask] = 0

    # Compact IDs: 0 stays 0, valid patches get sequential IDs from 1
...
def save_patch_raster(labeled: np.ndarray, transform: Affine, crs, out_path: Path):
    profile = {
        "driver": "GTiff",
        "dtype": "uint32",
        "nodata": 0,
...
def _stats_for_unit(geom, forest: np.ndarray, valid: np.ndarray,
    ...
    """Compute fragmentation stats for one admin unit geometry."""
    shape = forest.shape
    ...
    if forest_px == 0:
        return dict(area_ha=area_px * pixel_ha, forest_ha=0.0, pct_forest=0.0,
    ...
    sizes = np.bincount(local_labeled.ravel())[1:]  # skip background (0)
...
def compute_admin_stats(gdf: gpd.GeoDataFrame, forest: np.ndarray, valid: np.ndarray,
    ...
    for i, (_, row) in enumerate(gdf.iterrows(), 1):
        ...
        stats = _stats_for_unit(row.geometry, forest, valid, transform, pixel_ha, min_patch_px)
        rows.append(stats)
    print()
    stats_df = gpd.GeoDataFrame(rows, index=gdf.index)
...
def parse_args():
    p = argparse.ArgumentParser(description="Forest patch detection for Cameroon")
    p.add_argument("--treecover", type=Path, default=_DEFAULT_TREECOVER,
                   help="Input treecover GeoTIFF (default: cameroon_treecover2024_30m.tif)")
    p.add_argument("--threshold", type=int, default=10,
                   help="Forest threshold %% (default: 10)")
    p.add_argument("--min-patch-size", type=float, default=1.0,
                   help="Minimum patch size in ha (default: 1)")
    p.add_argument("--admin-level", type=int, choices=[0, 1, 2, 3], default=2,
                   help="Admin aggregation level 0–3 (default: 2)")
    p.add_argument("--burn-paved", action="store_true",
                   help="Treat paved roads as non-forest barriers")
    p.add_argument("--burn-unpaved", action="store_true",
                   help="Treat unpaved roads as non-forest barriers")
    p.add_argument("--roads", type=Path, default=_DEFAULT_ROADS,
                   help="Road GeoPackage with liu_surface column")
...
def main():
    ...
    if args.burn_unpaved:
        road_types.append("unpaved")

    roads_suffix = ""
    if road_types:
        roads_suffix = "__roads_" + "_".join(road_types)

    stem = (f"forest_patches"
            f"__thresh{args.threshold}"
            f"__minpatch{args.min_patch_size:.0f}ha"
            f"__adm{args.admin_level}"
            f"{roads_suffix}")
    tif_out = args.output_dir / f"{stem}.tif"
    gpkg_out = args.output_dir / f"{stem}.gpkg"

    # ------------------------------------------------------------------
    print("Loading treecover at 90 m …")
    arr, transform, crs, nodata = load_treecover_90m(args.treecover)
    pixel_ha = compute_pixel_area_ha(transform)
    min_patch_px = max(1, math.ceil(args.min_patch_size / pixel_ha))
    print(f"  Pixel area: {pixel_ha:.4f} ha  |  min patch: {min_patch_px} px "
          f"({args.min_patch_size} ha)")

    # ------------------------------------------------------------------
    print(f"Creating forest binary (threshold ≥ {args.threshold}%) …")
    forest, valid = make_forest_binary(arr, args.threshold)
    print(f"  Forest cover: {100 * forest.sum() / valid.sum():.1f}% of valid area")

    # ------------------------------------------------------------------
    if road_types:
        print(f"Burning roads ({', '.join(road_types)}) …")
        forest, valid = burn_roads(forest, valid, transform, crs, args.roads, road_types)
        print(f"  Forest cover after roads: {100 * forest.sum() / valid.sum():.1f}% of valid area")

    # ------------------------------------------------------------------
    print("Labeling country-level forest patches …")
    labeled = label_patches_country(forest, min_patch_px)
    n_country = int(labeled.max())
    print(f"  {n_country} patches after filtering < {args.min_patch_size} ha")

    save_patch_raster(labeled, transform, crs, tif_out)

    # ------------------------------------------------------------------
    admin_path = _ADMIN_DIR / f"cmr_admin{args.admin_level}.shp"
    ...
    gdf_stats = compute_admin_stats(gdf, forest, valid, transform, pixel_ha,
                                    min_patch_px, args.admin_level)

    gdf_stats.to_file(gpkg_out, driver="GPKG")
    print(f"  Saved stats → {gpkg_out.name}")

    # ------------------------------------------------------------------
    name_col = _ADMIN_NAME_COL[args.admin_level]
    print(f"\nTop 5 by forest cover (adm{args.admin_level}):")
