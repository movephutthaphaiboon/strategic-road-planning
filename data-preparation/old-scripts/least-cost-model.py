...
all_mines_df = pd.read_csv(MINES_FP)
all_mine_ids = all_mines_df["ID"].tolist()
early_stage_mine_ids = all_mines_df[all_mines_df["DEV_STAGE_AGGREGATED_SNL"] == "Early-stage"]["ID"].tolist()
late_stage_mine_ids  = all_mines_df[all_mines_df["DEV_STAGE_AGGREGATED_SNL"] == "Late-stage"]["ID"].tolist()
selected_mine_ids = late_stage_mine_ids #["cmr_027"]
...
def save_results(result_gdf: gpd.GeoDataFrame,
    ...
    out_best = OUTPUT_DIR / f"mine_to_port_paths_ds{downsample}_nmine{len(selected_mine_ids)}.gpkg"
    if not valid.empty:
    ...
    if multi_port:
        ...
        out_all = OUTPUT_DIR / f"mine_to_port_all_paths_ds{downsample}_nmine{len(selected_mine_ids)}.gpkg"
